...
function makeTwo() {
  ...
  fetch("https://emojihub.yurace.pro/api/random/group/face-positive")
    ...
    .then((data) => {
      result2.textContent = data.htmlCode;
    })
    .catch((err) => {
...
function makeSeven() {
  fetch("https://api.agify.io/").then((res) => {
    console.log(res); //422 unprocessable content
  });
}
...
function makeTen() {
  fetch("https://dog.ceo/api/breeds/image/random")
    ...
    .then((data) => {
      ...
      imageBlock.src = message;
      resultElement.append(imageBlock);
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    });
...
function makeEleven() {
  fetch("https://api.ipify.org?format=json")
    ...
    .then((data) => {
      ...
      resultElement.textContent = JSON.stringify(data.ip);
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    });
...
function makeTwelve() {
  ...
  fetch("http://ip-api.com/json/" + inputIP.value)
    .then((response) => response.json())
    .then((data) => {
      resultElement.textContent = JSON.stringify(data.country);
    })
    .catch((error) => {
      console.error("Ошибка:", error);
    });
